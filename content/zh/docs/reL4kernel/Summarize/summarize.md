...
## 2 微内核架构的设计原则
### 2.1 最小化内核原则
阐述微内核在内核中尽量只存放最必要的基本操作，而让内核其他部分都放到用户态的设计原则，及这种原则在安全性上的作用
### 2.2 策略与机制分离原则
阐述在内核中只保留机制，给出用户态的机制使用接口，并在用户态实施策略的设计原则。
### 2.3 实施细粒度的控制
阐述在微内核中对每一个内核对象都实施细粒度的控制以提高安全性的设计原则
### 2.4 系统服务解耦原则
阐述尽可能地解耦在用户态的系统服务，（也许可以包含内核态组件化？）

## 3 seL4内核概述
### 3.1 基于capability的内核对象抽象
...
#### 3.3.2 endpoint和notification消息传递
...
这种单内核的架构，也意味着不允许在内核态存在mutex等阻塞方法。具体来说，在宏内核中，如果某个进程A发起系统调用，例如申请磁盘读写操作，那么在执行到某个步骤的时候需要等待磁盘操作完成，这时候可以手动调用调度器切换到其他进程B，此时，进程A的内核栈中存放着它在内核中的执行步骤。

但是如果在单内核栈中，如果允许上述的步骤，进程A的运行栈帧之下一定存放了进程B的栈帧，此时回到进程B的用户态并再次进入内核态，就会破坏进程A的栈帧内容。

因此，在这样的设计下的每一次内核执行过程一定是串行而不允许中间的阻塞的。这也决定了，所有的内核的路径都是确定的，其一定存在某一次调用路径的栈空间最大，因此seL4只需要一个确定的足够大的栈空间即可，而无需产生任何动态申请内核栈空间的操作。这一点和上述的内核固定内存分配有着密切的联系。

对于seL4的调度模型，在每个进入内核的路径中，都会在某些必要的时候将当前进程放置到调度队列中，而在离开内核的时候调用schedule函数查看目前应该选择的进程。

### 3.5 seL4的中断和异常处理

## 4 使用Rust语言重写seL4——reL4内核的实现
### 4.1 reL4项目概述
...
### 4.6 支持reL4构建的辅助工具实现
##### 4.6.1 reL4的xtask构建脚本支持

##### 4.6.2 reL4的pbf文件格式解析支持
