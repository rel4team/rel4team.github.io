...
最后一种状态相对复杂，seL4将其进行了简化。每次接到中断之后都会直接清除该中断的允许位，让他必须在用户态调用了AckIRQ的系统调用之后，才能够重新打开该中断的允许位，这又是一个策略和机制分离的重要体现，对中断的确认放置到了用户态来进行。这也直接导致了，第四种状态的未被使用。
